...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            # Affine
            out, cache = affine_forward(input_data, self.params['W' + str(i + 1)], self.params['b' + str(i + 1)])
        ...
        # Affine
        dout, dw, db = affine_backward(dout, affine_caches[self.num_layers - 1])
        ...
        for i in reversed(range(self.num_layers - 1)):
            ...
            if self.normalization != None:
                if self.normalization == 'batchnorm':
                    dout, dgamma, dbeta = batchnorm_backward_alt(dout, normal_caches[i])
                    grads['gamma' + str(i + 1)] = dgamma
                    grads['beta' + str(i + 1)] = dbeta
                elif self.normalization == 'layernorm':
                    dout, dgamma, dbeta = layernorm_backward(dout, normal_caches[i])
                    grads['gamma' + str(i + 1)] = dgamma
                    grads['beta' + str(i + 1)] = dbeta
                else:
                    print('Wrong normalization string')
            # Affine
            dout, dw, db = affine_backward(dout, affine_caches[i])
