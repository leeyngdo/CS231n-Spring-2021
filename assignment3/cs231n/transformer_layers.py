...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        columns = torch.arange(0, max_len).view(-1, 1)
        rows = 10000 ** (-1 * (torch.arange(0, embed_dim, 2) / embed_dim))
        
        pe[:, :, 0::2] = torch.sin(columns * rows)
        pe[:, :, 1::2] = torch.cos(columns * rows)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        scale = torch.sqrt(torch.FloatTensor([D / H]))
        ...
        similarities = torch.matmul(Q, K.transpose(2, 3)) / scale 
        if attn_mask is not None:
            similarities.masked_fill_(attn_mask == 0, -float('inf'))
            
        attention = self.dropout(F.softmax(similarities, dim = 3))
        
        output = torch.matmul(attention, V)
        # refer to https://f-future.tistory.com/entry/Pytorch-Contiguous #
        output = output.transpose(1, 2).contiguous()
        output = self.proj(output.view(N, -1, D))
