...
def sample_noise(batch_size, dim, seed=None):
    ...
    # changing the range of distribution #
...
def discriminator_loss(logits_real, logits_fake):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N = logits_real.size()
    real = torch.ones(N).type(dtype)
    fake = 1 - real
    loss = bce_loss(logits_real, real) + bce_loss(logits_fake, fake)
...
def generator_loss(logits_fake):
    ...
    N = logits_fake.size()
    fake = torch.ones(N).type(dtype)
    loss = bce_loss(logits_fake, fake)
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss = torch.mean(0.5 * (scores_real - 1) ** 2) + torch.mean(0.5 * scores_fake ** 2)
