...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        input_x = x[:, i, :]
        prev_h, cache_i = rnn_step_forward(input_x, prev_h, Wx, Wh, b)
        h[:, i, :] = prev_h
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    a_i, a_f, a_o, a_g = activation[:, 0 : H], activation[:, H : 2*H], activation[:, 2*H : 3*H], activation[:, 3*H : 4*H]
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dactivation = np.hstack([da_i, da_f, da_o, da_g])
...
def lstm_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        cache_t = cache[t]
        dnext_h += dh[:, t, :]
        dx_t, dnext_h, dnext_c, dWx_t, dWh_t, db_t = lstm_step_backward(dnext_h, dnext_c, cache_t)
