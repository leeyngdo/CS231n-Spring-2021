...
def sim(z_i, z_j):
    ...
    ##############################################################################
    norm_z_i = z_i / torch.linalg.norm(z_i)
    norm_z_j = z_j / torch.linalg.norm(z_j)
    norm_dot_product = torch.dot(norm_z_i, norm_z_j)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        l_k = -torch.log(numerator / denominator_k)
        l_k_N = -torch.log(numerator / denominator_k_N)
...
def sim_positive_pairs(out_left, out_right):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N, _ = out_left.shape
    norm_left = out_left / torch.linalg.norm(out_left, dim = 1, keepdims = True)
    norm_right = out_right / torch.linalg.norm(out_right, dim = 1, keepdims = True)
    pos_pairs = torch.diag(torch.matmul(norm_left, norm_right.transpose(0,1)), 0).view(-1, 1)
...
def compute_sim_matrix(out):
    ...
    norm_out = out / torch.linalg.norm(out, dim = 1, keepdims = True)
    sim_matrix = torch.matmul(norm_out, norm_out.transpose(0, 1))
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda:1'):
    ...
    numerator_matrix = torch.gather(sim_matrix.to(device), 1, torch.tensor(range(N, 2*N)).to(device).view(-1, 1))
    numerator_matrix = torch.vstack([numerator_matrix , numerator_matrix])
    ...
    numerator = None
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    numerator = torch.exp(numerator_matrix / tau).to(device)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
